...
def usage():
    print "usage: %s [-count N] [-stdout] [-append]" % sys.argv[0]
...
if not output:
    output = open("/tmp/access.log", outputMode)
...
import json
import string
url = 'http://localhost:9501/?channel=b955e4e7-a48b-4097-9dec-a599cd51b413'
#url2 = 'http://localhost:9501/?channel=b3367f60-34e5-44d3-bdc4-7561fcb28ad2'
while count < 0 or numMsgs < count:
    ...
    (http_verb,resource,http_version,http_status,num_bytes) = string.split(currDoc)
    
    logRecordJSON = ''
    if num_bytes == '-':
        logRecord = { 'docType' : 'apacheAccess', 'clientIP' : ips[n], 'ident' : '-', 'remoteUser' : '-', 'time' : t, 'resource':docs[m], 'http_status' : http_status}
        logRecordJSON = json.dumps(logRecord)
    else:
        logRecord = { 'docType' : 'apacheAccess', 'clientIP' : ips[n], 'ident' : '-', 'remoteUser' : '-', 'time' : t, 'resource':docs[m], 'http_status' : http_status, 'bytes' : int(num_bytes) }
        logRecordJSON = json.dumps(logRecord)
    headers = {'content-type' : 'application/json' } 
    r = requests.post(url, data=logRecordJSON, headers=headers)
    #r1 = requests.post(url2 , data=logRecordJSON, headers=headers)
    print >>output, "%s - - %s %s %s %s" % (ips[n],date,docs[m],'"http://aa"', '"Mozilla"')
    output.flush()
